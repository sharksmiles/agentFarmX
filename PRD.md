...
> **版本**: v3.1（Agent 付款与技能系统完整版）| **更新日期**: 2026-03-21 | **语言**: 简体中文
> **基于代码库分析**: 完整分析 Prisma Schema、API Routes、Components、Utils、agentExecutor、agentService
...
| **交易记录** | ✅ | ✅ | ✅ | 链上交易历史 |
| **x402 + Permit2 支付** | ✅ | ✅ | ✅ | 前端 Permit2 预授权 + 后端 transferFrom，每技能扣 0.001 USDC，`AgentPaymentAuth` 表存储授权状态 |
...
> **更新于 2026-03-21**：原有 5 项技术债务已全部修复。

| 优先级 | 状态 | 问题 | 修复说明 |
|--------|------|------|----------|
| 🟡 中 | ✅ 已修复 | 每日签到 7 天链无法追踪 | GET 接口新增 `checked_days[]`、`days_in_current_cycle`、`next_reward_day` 字段 |
| 🟡 中 | ✅ 已修复（早期）| 偷盗成功率逻辑不一致 | `checksteal` 与 `steal` 均已统一使用 `calculateStealSuccessRate()` |
| 🟡 中 | ✅ 已修复 | agentService 决策逻辑固定 | Raider Bot 70/15/10/5% 概率分配；Farming Bot 随机选地块和作物、10% 概率运行策略分析 |
| 🟢 低 | ✅ 已修复 | `buy_seed` 缺少库存感知 | 执行前查询 Inventory 表，库存 ≥ 5 颗则跳过购买 |
| 🟢 低 | ✅ 已修复 | 部分 API 错误响应格式不统一 | `visit`、`tasks/daily` 路由统一使用 `errorResponse` / `internalErrorResponse` 辅助函数 |
...
| Permit2 | `0x000000000022D473030F116dDEE9F6B43aC78BA3` | Uniswap Permit2（Agent 预授权）|
...
- **支付**: x402 HTTP 支付协议 + Uniswap Permit2（EIP-712 预授权 + 链上 transferFrom）
- **链上交互**: Ethers.js v6
...
**x402 + Permit2 支付**:
- ✅ 前端 Permit2 预授权流程（`permit2Preauth()`）
- ✅ 首次 USDC → Permit2 合约 ERC-20 approve（仅一次）
- ✅ 每次启动 EIP-712 PermitSingle 签名（gasless）
- ✅ 后端提交 Permit2 permit 上链（后端付 gas）
- ✅ 每技能执行：后端调用 `permit2TransferFrom` 划扣 0.001 USDC
- ✅ `agentPaymentAuth` 数据库余额追踪
- ✅ 内部 Agent 调用通过 `x-user-id` 绕过 x402 检查（`isInternal` 标志）

**实现位置**:
- `src/utils/func/permit2.ts` - 前端 Permit2 流程
- `src/utils/func/x402.ts` - x402 协议
- `src/utils/func/walletAuth.ts` - 钱包认证
- `src/services/permit2Service.ts` - 后端 Permit2 服务
- `src/middleware/auth.ts` - `withAuth` 支持 `x-user-id` 内部调用
- `src/app/api/agents/[id]/preauth/permit2/route.ts` - 签名确认 API
...
**内置技能列表（12个，通过 `scripts/seed-skills.ts` 初始化）**：

| 技能名 | 分类 | 价格 | 实现方式 |
|--------|------|------|----------|
| `plant_crop` | farming | 0.001 USDC | `POST /api/farm/plant` |
| `harvest_crop` | farming | 0.001 USDC | `POST /api/farm/harvest` |
| `use_boost` | farming | 0.001 USDC | `POST /api/farm/boost`（自动选非成熟地块）|
| `unlock_land` | farming | 0.001 USDC | `POST /api/farm/unlock` |
| `buy_seed` | farming | 0.001 USDC | `POST /api/shop/buy` |
| `steal_crop` | social | 0.001 USDC | `POST /api/social/steal`（自动查找成熟目标）|
| `visit_friend` | social | 0.001 USDC | `POST /api/social/visit` |
| `water_friend_crop` | social | 0.001 USDC | `POST /api/social/water`（自动查找可浇水目标）|
| `radar_scan` | social | 0.001 USDC | DB 直查（无 HTTP 端点）|
| `check_energy` | strategy | 0.001 USDC | DB 直查 FarmState |
| `optimize_farm` | strategy | 免费 | DB 直查 CropConfig |
| `analyze_market` | strategy | 免费 | DB 直查 User + FarmState |
...
| 技能名称 | 分类 | 描述 | Energy | USDC 费用 |
| :--- | :--- | :--- | :--- | :--- |
| `plant_crop` | Farming | 在指定地块种植作物 | 10 | 0.001 |
| `harvest_crop` | Farming | 收割成熟作物 | 5 | 0.001 |
| `unlock_land` | Farming | 解锁/购买新地块 | 0 | 0.001 |
| `use_boost` | Farming | 对非成熟地块使用加速道具 | 0 | 0.001 |
| `buy_seed` | Farming | 从商店购买种子 | 0 | 0.001 |
| `visit_friend` | Social | 访问好友农场获取经验 | 5 | 0.001 |
| `water_friend_crop`| Social | 给好友作物浇水 | 10 | 0.001 |
| `steal_crop` | Social | 尝试偷取好友成熟作物（自动寻找目标）| 15 | 0.001 |
| `radar_scan` | Social | 扫描附近有成熟作物的农场 | 0 | 0.001 |
| `analyze_market` | Strategy | 分析用户状态与风险 | 0 | 免费 |
| `optimize_farm` | Strategy | 分析作物收益，给出种植建议 | 0 | 免费 |
| `check_energy` | Strategy | 检查能量值和恢复进度 | 0 | 0.001 |
...
- **Token 消耗**: 每次 LLM 决策消耗 Token，记录在 `AgentDecision.cost`（模拟模式下无 Token 消耗）。
- **USDC 消耗**: 每个付费技能扣 **0.001 USDC**，通过 Permit2 `transferFrom` 从用户钱包划扣。
  - 运行频率：每 5 分钟 1 次决策 = **0.012 USDC/小时** / **0.288 USDC/天**
  - 预授权额度：1 USDC ≈ 1000 次技能 ≈ **3.5 天**
  - 余额耗尽后：技能返回 `INSUFFICIENT_PREAUTH_BALANCE`，机器人停止
- **Gas 消耗**: Permit2 permit 提交 + transferFrom 由后端钱包支付 gas，用户无需持有 OKB。
- **Energy 消耗**: 技能执行消耗游戏内 Energy（自动恢复）。

### 6.4 创建 Agent（`/agents/create`）✅ 已更新

**新版引导流程**：
1.  **Persona**: 输入名称，选择性格 (Aggressive/Conservative/Balanced) 和策略 (**Farming/Raider**)。
