...
| **能量系统** | ✅ | ✅ | ✅ | 自动恢复 + 购买套餐 |
| **交易记录** | ✅ | ✅ | ✅ | 链上交易历史 |
| **x402 支付** | ✅ | ⚠️ | N/A | 前端拦截器完成，后端验证待实现 |
| **商店系统** | ✅ | ✅ | ✅ | 种子购买 API 已实现 (`/api/shop/buy`) |
...
| 任务 | 路径 | 频率 | 功能 |
|------|------|------|------|
| 能量恢复 | `/api/cron/energy-recovery` | 每分钟 | 批量恢复用户能量 |
...
| 优先级 | 问题 | 影响 | 位置 |
|--------|------|------|------|
| 🟡 中 | x402 后端验证逻辑未实现 | Radar 功能无法完整闭环 | `/api/payment/*` |
| 🟡 中 | GameStats 使用 Mock 数据 | 作物配置未从数据库读取 | `utils/api/game.ts:8-11` |
...
| Agent Factory | 待部署 | ERC-4337 SCA 工厂 |
| $FARM Token | 待部署 | 治理代币 (ERC-20) |
| Raffle | 待部署 | 抽奖合约 |
