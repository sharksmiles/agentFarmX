...
> **截止**: 2026-03-14，后端 API 集成 **7 个阶段全部完成**，TypeScript 构建通过（exit 0）。

| 模块 | 状态 | 说明 |
|------|------|------|
| 前端 UI | ✅ 100% | 全部路由与交互已实现 |
| 后端 API 对接（Phase 1~7） | ✅ 完成 | `src/utils/api/` 下 9 个模块全部对接真实端点 |
...
| x402 支付协议 | ✅ 已实现 | `utils/func/x402.ts` + `api/client.ts` 拦截器自动处理 402 重试 |
...
| `/g/raffle/{id}/participants/?cursor=` | GET | 参与者列表（Cursor 分页） |
...
| `/u/friends/{id}/farm/` | GET | 获取好友农场状态（`FriendStats`） |
| `/u/friends/requests/` | GET | 入站好友请求列表 |
...
| `/u/airdrop/` | POST | `{airdrop_id}` 领取空投，返回 `{tx_hash}` |
| `/u/balances/` | GET | 查询链上余额（OKB / COIN / Stone / Crystal / ART） |
...
| `/api/agents/{id}/control/` | POST | `{action:"start"\|"pause"\|"stop"}` — 统一控制端点 |
| `/api/agents/{id}/topup/` | POST | `{token, amount, tx_hash}` 记录链上充値 |
| `/api/agents/{id}/logs/?cursor=` | GET | 日志列表（支持 filter + cursor 分页） |
| `/api/agents/preview-sca/` | POST | `{type}` 预查新 Agent 的 SCA 地址 |
| `/api/services/` | GET | 服务市场列表（预留，待 Phase 8） |
...
| 优先级 | 问题 | 位置 |
|--------|------|------|
| 🔴 高 | 需测试 Agent SCA 充値完整链上流程（`onchain.ts` 转账 + `recordAgentTopup` 对接） | `utils/func/onchain.ts` |
...
| 🟢 低 | 清理残余 lint 警告 | 全局 |
| 🟢 低 | 移除遗留 BIP39 钱包逻辑（`/new-wallet` 路由及 `utils/encryption/`） | `/new-wallet` |
| 🟢 低 | `fetchOnChainBalances` （`/u/balances/`）尚未在 Wallet 页展示链上余额 | `components/wallet/` |

**已完成项目（不再列出）**：✅ SIWE 登录 | ✅ 全部 MOCK_* 替换为真实 API | ✅ Buffer 构建报错修复 | ✅ x402 自定义实现 | ✅ Telegram 集成移除 | ✅ Capila NFT 功能移除
