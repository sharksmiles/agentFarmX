...
async function main() {
  ...
  if (farmState) {
    ...
    // Raider Bot 策略：优先偷菜
    if (agent.strategyType === 'raider' || agent.strategyType === 'aggressive') {
      if (farmState.energy >= 20) {
    ...
    if (skill) {
      ...
      if (skill.priceUsdc && skill.priceUsdc > 0) {
        if (agent.paymentAuths && agent.paymentAuths.length > 0) {
          ...
          if (remaining >= skill.priceUsdc) {
            log('green', `   ✅ 预授权余额充足 (${remaining.toFixed(4)} USDC)`);
          } else {
            log('red', `   ❌ 预授权余额不足 (${remaining.toFixed(4)} USDC)`);
          }
        } else {
          log('red', `   ❌ 付费技能需要预授权!`);
        }
